...
class LPCI:
    ...
    def gen_conf_interval(self, preds: np.array, quantiles: np.array):
        ...
        Always excludes the extreme columns q_0.05 (lower side) and q_1.0 (upper side).
        Assumes quantiles were generated by `gen_quantiles`, i.e., the first half are
        lower quantiles in ascending order [0, ..., alpha] and the second half are
        upper quantiles [1-alpha, ..., 1.0], paired by index.
        ...
        opt_lower_q: np.array
            The chosen lower quantile for each sample.

        opt_upper_q: np.array
            The chosen upper quantile for each sample.
        """

        # split the quantiles and predictions into lower and upper halves
        mid = int(len(quantiles) / 2)
        lower_qs = quantiles[:mid]
        upper_qs = quantiles[mid:]
        lower_preds = preds[:, :mid]
        upper_preds = preds[:, mid:]

        # build keep masks (exclude q_0.05 on lower side and q_1.0 on upper side)
        lower_keep = np.ones_like(lower_qs, dtype=bool)
        upper_keep = np.ones_like(upper_qs, dtype=bool)

        # exclude last lower if it's 0.05 (typical when alpha=0.05)
        if np.isclose(lower_qs[-1], 0.05, rtol=0.0, atol=1e-8):
            lower_keep[-1] = False

        # exclude last upper if it's 1.0 (always true with current gen_quantiles)
        if np.isclose(upper_qs[-1], 1.0, rtol=0.0, atol=1e-12):
            upper_keep[-1] = False

        # combine masks to keep pairing by index (both halves must have equal length)
        # (this works because the extremes we drop are at the same paired index, the last one)
        pair_keep = lower_keep & upper_keep

        # guardrail
        if not np.any(pair_keep):
            raise ValueError("No valid quantile pairs remain after excluding q_0.05 and q_1.0.")

        # apply mask
        lower_qs = lower_qs[pair_keep]
        upper_qs = upper_qs[pair_keep]
        lower_preds = lower_preds[:, pair_keep]
        upper_preds = upper_preds[:, pair_keep]

        # compute widths and pick the narrowest
        widths = upper_preds - lower_preds
        i_star = np.argmin(widths, axis=1)

        # gather optimal intervals and their quantiles
        rows = np.arange(len(i_star))
        lower_conf = lower_preds[rows, i_star]
        upper_conf = upper_preds[rows, i_star]
        opt_lower_q = lower_qs[i_star]
        opt_upper_q = upper_qs[i_star]
